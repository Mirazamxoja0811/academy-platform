              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input 
                    ...
                    className="w-full bg-black/40 border border-yellow-500/30 rounded-xl px-4 py-4 text-yellow-500 font-bold text-xl focus:outline-none focus:border-yellow-500 transition-colors placeholder:text-yellow-500/20"
                  />
                  {/* Quick buttons */}
      ...
      {showToast && (
        ...
        >
          <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-black px-10 py-6 rounded-3xl shadow-[0_0_100px_rgba(234,179,8,0.8)] font-black text-2xl flex flex-col items-center gap-4">
            ...
            {students.find(s => s.id.toString() === selectedStudent)?.full_name || ''} ga {amount} coin yuborildi!
          </div>
        </motion.div>
      )}
    </div>
